...
namespace Microsoft.Samples.Kinect.WpfViewers
{
    ...
    {
        static float refX, refY, refZ, tanVal, refHandY;
        static bool bl = false;
        static ComCon cc = new ComCon();
        ...
        private void DrawBonesAndJoints(DrawingContext drawingContext)
        {
            ...
            if (this.ShowJoints)
            {
                ...
                {
                    Joint myrightHand = this.currentSkeleton.Joints[JointType.Head];
                    Joint myStallRead = this.currentSkeleton.Joints[JointType.HandLeft];
                    //Joint myHitRead = this.currentSkeleton.Joints[JointType.HandRight];
                    // get the individual points of the right hand
                    float rightX = myrightHand.Position.X;
                    float rightY = myrightHand.Position.Y;
                    float rightZ = myrightHand.Position.Z;
                    //float hitZ = myHitRead.Position.Z;
                    float handY = myStallRead.Position.Y;
                    if (!bl)
                    {
                        refX = rightX;
                        refY = rightY;
                    ...
                    rightY = rightY - refY;
                    rightZ = rightZ - refZ;
                    handY = handY - refHandY;
                    //hitZ = hitZ - refHitZ;

                    tanVal = (float)Math.Atan2(rightZ, rightX);
                    if (tanVal < 0)
                    {
                        tanVal += (float)(Math.PI * 2);
                    }
                    ...
                    if (cc.isAvailable())
                    {
                        str = cc.readStr();
                        j = (int)(tanVal * 100);
                        if (Math.Sqrt(Math.Pow(rightZ, 2) + Math.Pow(rightX, 2)) < 0.1)
                        {
                            j = j + 1000;
                        }
                        else
                    ...
                    //else
                     ...
                     FormattedText ft =  new FormattedText(rightX + "\n" + rightY + "\n" + rightZ + "\n" + j + "\n",CultureInfo.GetCultureInfo("en-us"),FlowDirection.LeftToRight,new Typeface("Verdana"),36, System.Windows.Media.Brushes.Black);
                     Point pt =  new System.Windows.Point(200, 116);
